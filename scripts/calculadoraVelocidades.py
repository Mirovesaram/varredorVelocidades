    ...
    try:
        ...
        for i in range(numeroDeRepeticoes):
            ...
            for j in range(quantidadeLinhas):
                ...
                def distanciometro(dist):

                    distancia = dist
                    
                    maiorDistancia = abs(dataFrameVelocidades.vy.max() - dataFrameVelocidades.vy.min())

                    porcentagem = (distancia/maiorDistancia)

                    peso = 0

                    if porcentagem >= 0 and porcentagem <= 0.05:

                        peso = 1

                    elif porcentagem > 0.05 and porcentagem <= 0.1:

                        peso = 0.9

                    elif porcentagem > 0.1 and porcentagem <= 0.15:

                        peso = 0.8

                    elif porcentagem > 0.15 and porcentagem <= 0.2:

                        peso = 0.7

                    elif porcentagem > 0.2 and porcentagem <= 0.25:

                        peso = 0.6

                    elif porcentagem > 0.25 and porcentagem <= 0.30:

                        peso = 0.5

                    elif porcentagem > 0.3 and porcentagem <= 0.35:

                        peso = 0.4

                    elif porcentagem > 0.35 and porcentagem <= 0.4:

                        peso = 0.3

                    elif porcentagem > 0.4 and porcentagem <= 0.45:

                        peso = 0.2

                    elif porcentagem > 0.45 and porcentagem <= 0.5:
                ...
                def varredor(vel, dist):
                    ...
                    if velocidade > 0:
                        ...
                        ponto *= distanciometro(distancia)
                ...
                def varrerDianteira():
                    ...
                    if diferenca <= varredura:
                        ...
                        for k in range((varredura+1)-exclusao):
                            ...
                            distancia = abs(dataFrameVelocidades.iloc[(j+k),1] - dataFrameVelocidades.iloc[j,1])

                            pontuacao += varredor(dataFrameVelocidades.iloc[(j+k),1], distancia)
